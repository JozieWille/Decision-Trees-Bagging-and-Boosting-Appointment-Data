...
# Download Appointments.csv and read data
...
# predicting the class of each observation
tree.pred <- predict(tree.appt, type = "class")

# Reporting the predicted probability of belonging to each class
...
# plotting and labeling the tree
plot(tree.appt)
text(tree.appt, pretty = 0)
...
# Creating a confusion matrix using test data
...
# cross validation using cv.tree method
set.seed(200)

# 10-folds cross validation tree.train <- tree(Status~.-MRN,data=appt,split='gini')
...
# pruning tree to 2 terminal nodes
...
# bagging for appt dataset by setting mtry equal to the number of predictor variables 
